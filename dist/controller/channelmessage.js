...
exports.default = function (_ref) {
  ...
  api.post('/add', _authMiddleware.authenticate, function (req, res) {
    ...
    newMessage.save(function (err) {
      ...
      res.status(200).json({ message: 'Channel Message saved successfully' });
    });
  });

  // '/v1/message/:id' - Update
  api.put('/:id', _authMiddleware.authenticate, function (req, res) {
    _channelmessage2.default.findById(req.params.id, function (err, message) {
      if (err) {
        res.status(500).json({ message: err });
      }
  ...
  api.get('/byChannel/:channelId', _authMiddleware.authenticate, function (req, res) {
    _channelmessage2.default.find({ 'channelId': req.params.channelId }, function (err, messages) {
      if (err) {
        res.status(500).json({ message: err });
        return;
      }
  ...
  api.delete('/:id', _authMiddleware.authenticate, function (req, res) {
    _channelmessage2.default.remove({
      _id: req.params.id
  ...
  api.delete('/', _authMiddleware.authenticate, function (req, res) {
    _channelmessage2.default.find({}, function (err, users) {
      if (err) {
        res.status(500).json({ message: err });
        return;
      }
